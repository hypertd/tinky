...
class Shell extends \Psy\Shell
{
    ...
    public function __construct(Configuration $config = null)
    {
        ...
        Application::__construct('Tinky Shell', self::VERSION);
    ...
    {
        if ($ret = Application::add($command)) {
   ...
   /*
     * Same as Application with custom shellinput rules that allow for passthrough commands
     */
    protected function runCommand($input)
    {
        ...
        if($command instanceof \Tinky\Command\PassthroughCommand){
            $input = new \Tinky\Input\PassthroughShellInput(str_replace('\\', '\\\\', rtrim($input, " \t\n\r\0\x0B;")));
        }
        else{
    ...
    {
        return sprintf('<aside>%s by Matt Burgess</aside>', $this->getVersion());
    }
    ...
    public function getVersion()
    {
        ...
        return sprintf('Tinky Shell %s (PHP %s %s %s)', self::VERSION, phpversion(), $separator, php_sapi_name());
    }
